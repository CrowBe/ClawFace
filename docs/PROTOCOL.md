...
The discovery script (`scripts/openclaw-gateway-discover.js`) validates this handshake shape. By default it runs read-only probes and calls `sessions.preview` only with keys returned by `sessions.list`, avoiding invalid empty-key requests. When `OPENCLAW_GATEWAY_SEND_TEXT` is set, it also performs an explicit write-scope `sessions.send` round-trip probe and prints summarized event shapes for a provided or newly-created target session without logging message content. When `OPENCLAW_GATEWAY_REVOKE_DEVICE_TOKEN=1` is set with pairing scope, it calls `device.token.revoke` for the probe's signed device identity, then verifies the revoked issued token is rejected on reconnect. The app transport (`services/transport/openclaw-gateway.ts`) implements challenge handling, mobile Ed25519 device signing, token/device-token authentication with SecureStore persistence, and signed-device token revocation.
...
The discovery script defaults to `operator.read`. Set `OPENCLAW_GATEWAY_PRINT_FULL_FEATURES=1` to print the full advertised method/event list, or `OPENCLAW_GATEWAY_SCOPES=operator.read,operator.write,operator.pairing,operator.approvals` to inspect a broader scope set. It only sends a write probe when `OPENCLAW_GATEWAY_SEND_TEXT` is provided. The app transport defaults to `operator.read,operator.write,operator.pairing,operator.approvals` so it can self-revoke paired device tokens when a signed device identity is available and bridge runtime approval requests/decisions (CF-015). `operator.admin` is not required for the M1 command surface and should not be a default.
...
| Resolve approvals | `exec.approval.resolve`, `plugin.approval.resolve` | Requires `operator.approvals`. Wired (CF-015): the app transport tracks each approval's kind (exec vs plugin) from the inbound `*.approval.requested` event and resolves with `{ requestId, decision: 'approve' \| 'deny', idempotencyKey }`. The param schema is OpenClaw-owned; the field names here are a defensive best-effort pending live-Gateway confirmation. |
...
| Exec approval cards | `exec.approval.requested`, `exec.approval.resolved` | Requires `operator.approvals`. Wired (CF-015): `requested` normalizes into a ClawFace `approval_request` (pending card keyed by the opaque approval request id, routed by `sessionKey`, with a default 5-minute expiry if none is supplied); `resolved` replays the resolved status onto the existing card without re-notifying. Field extraction is defensive (see `normalize.ts`). |
| Plugin approval cards | `plugin.approval.requested`, `plugin.approval.resolved` | Requires `operator.approvals`. Same handling as exec approval cards; resolution routes to `plugin.approval.resolve`. |
