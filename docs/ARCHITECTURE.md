...
- The Gateway transport implementation covers connect, mobile Ed25519 device signing, SecureStore device-token persistence, send, subscribe, thread creation, session list hydration, Gateway-derived Agent Context, `hello-ok.policy` limit/tick consumption, approval bridging (CF-015: `exec`/`plugin` `approval.requested`/`resolved` events → approval cards, approve/deny via `exec.approval.resolve`/`plugin.approval.resolve`), and connected-device token revocation via `device.token.revoke`. Remaining CF-026 work is full mobile-app validation against a real local OpenClaw Gateway — including confirming the defensively-extracted approval payload field names — and any scoped presence-beacon polish.
